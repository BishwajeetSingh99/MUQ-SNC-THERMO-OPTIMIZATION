...
def sample_plot_all_species(
    unsrt_data, design_matrix, outdir="sample_plots", seed=None
):
    """Same high‑level purpose as before, but styled for Elsevier multi-panel layout

    (no legend, no title, no tick marks, sized for 3-side-by-side subfigures).
    """
    # ---------------------------------------------------------------------
    ...
    chosen_rows = np.random.choice(
        design_matrix.shape[0], size=300, replace=False
    )
    ...
    for row_idx in chosen_rows:
        ...
        for sp_idx, sp in enumerate(species_order):
            ...
            # ── plotting ────────────────────────────────────────────────
            # Sized for 3 figures side-by-side in LaTeX (~3.5 inches width)
            ...
            # 2. Hide tick mark lines completely while preserving axis labels
            ax.tick_params(
                axis="both",
                which="both",
                length=0,  # removes tick mark lines
                top=False,
                right=False,
                labelsize=9,
            )
            ...
            # 3. Plots
            ax.plot(T_lo, cp_lo, color="red", linewidth=1.5)
            ax.plot(T_hi, cp_hi, color="green", linewidth=1.5)

            ax.plot(T_lo, c["nom_lo"], color="blue", linewidth=1.8)
            ax.plot(T_hi, c["nom_hi"], color="blue", linewidth=1.8)

            ax.plot(T_lo, c["env_lo_hi"], "k--", linewidth=1.0)
            ax.plot(T_lo, c["env_lo_lo"], "k--", linewidth=1.0)
            ax.plot(T_hi, c["env_hi_hi"], "k--", linewidth=1.0)
            ax.plot(T_hi, c["env_hi_lo"], "k--", linewidth=1.0)

            # 4. Label Quality Tweaks
            #ax.set_xlabel("Temperature (K)", fontsize=10, labelpad=6)
            #ax.set_ylabel(   r"$C_p$ (J mol$^{-1}$ K$^{-1}$)", fontsize=10, labelpad=6)
            ax.set_ylim(
                0.92 * c["env_lo_lo"].min(), 1.15 * c["env_hi_hi"].max()
            )
    ...
    print(
        f"✓ 100 rows × {n_sp} species ⇒ {len(chosen_rows)*n_sp} Cp‑plots "
